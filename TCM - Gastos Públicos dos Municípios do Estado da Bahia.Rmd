...
# Função que cria o formato do log (tempo de execução das funções para registro no BD)
log_data_hora()

# Função que cria o SQLite e a respectiva conexão.
conexao_sgbd()

# Função que cria 4 das 5 tabelas que serão armazenadas no SQLite. A 5ª será criada em tempo de execução
# para registrar log, se for necessário
criar_tabelas_bd()
...
# Função faz o pré-processamento dos dados do HTML, aplicando o conceito Tidy Data de Hadley Wickham (http://r4ds.had.co.nz/tidy-data.html) . Com isso, os dados são facilmente analisados em software de
# Business Intelligence ou de estatística como o próprio R.
