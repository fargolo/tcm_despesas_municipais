...
criar_tb_entidades_alvo <- function(ano, cod_municipios) {
...
executar_tidy_data <- function() {
  ...
  tb_gastos_municipios <- DBI::dbReadTable(conect_bd, "tabela_gastos_municipios") %>%
                          ...
                          #dplyr::mutate_if(valor_das_retencoes = "-", 0)
...
# que têm os links que derão acesso as páginas HTML que contêm os dados sobre as despesas
criar_tb_entidades_alvo(ano, cod_municipios)
...
# MELHORIAS
#!!! Colocar a função 'scraping_html_purrr' no padrão da Programação Funcional
...
#!!! Transformar os valor "-" na coluna `valor_das_retencoes` em 0
...
#!!! Evitar duplicidade de dados na hora de raspar os HTMLs das páginas das entidades.
#    Verificar se a duplicidade de dados deve ser abordada no Scraping ou na etapa de Pré-Processamento
#!!! Aperfeiçoar a função `valor_monetario()` para não tranformar os valor "-" em NA.
...
item <- c("a", "b", "c", "d", "e")
valor <- c("R$86.098,02", "-", "R$ 589.256,36", "-", "-")

tab <- tibble(item = item,
              valor = valor)
