---
title: "Web Scraping - Despesas dos Municípios do Estado da Bahia"
author: "George Santiago"
date: "18 de jlnho de 2018"
...
tb_dcalendario <- map_dfr(anos_alvos, tb_anos_alvos) %>%
                  tibble::as.tibble()
...
executar_scraping_num_pags <- function(anos_alvos, cod_municipios_alvos) {
...
tb_municipios_alvos_novos <- DBI::dbReadTable(conect_bd, "tabela_tcm_dmunicipios_entidades") %>%
                             ...
                             dplyr::mutate(ano = anos_alvos) %>%
                             dplyr::select("ano", "cod_municipio", "nm_municipio",
                                           "cod_entidade", "nm_entidade", "pagina")

tb_municipios_alvos_anteriores <- DBI::dbReadTable(conect_bd, "tabela_entidades_alvos_paginas")

cod_mun_ant <- tb_municipios_alvos_anteriores$cod_municipio

ano_ant <- tb_municipios_alvos_anteriores$ano %>%
           unique()
...
tb_municipios_alvos_atualizada <- tb_municipios_alvos_novos %>%
                                  ...
                                  dplyr::filter(!cod_municipio %in% cod_mun_ant & !ano %in% ano_ant)
...
if (length(scraping_html$result) == 0) {
    ...
    print(paste("### Timeout estrapolou o tempo", "-", nm_arq_html_pag,
                "- doc:", documento, "- primeira tentativa ###"))
...
if (length(scraping_html$result) == 0) {
    ...
    tb_request <- tibble::tibble(
     ...
     return(print(paste("### Timeout estrapolou o tempo", "-", nm_arq_html_pag,
                        "- doc:", documento, "- segunda tentativa ###")))
...
if (scraping_html$result$status_code == 404) {
   ...
   # Parar a iteração e pular para a próxima requisição.
   return(print(paste("### Erro 404 de Requisição", "-",
...
if (teste_html_despesas[8] == teste_1) {
  ...
  # a informação '-' na base de dados do TCM
    ...
    tb_request <- tibble::tibble(
                                 log = "HTML de despesa incompleto",
                                 time = log_request,
                                 id = id,
                                 nm_entidade = nm_entidade,
                                 pagina = pagina,
   ...
   return(print(paste("### O HTML", nm_arq_html_pag, "- doc:", documento, "não está com informações completas. Tentar mais tarde. ###")))
  ...
  # Salva o arquivo HTML no HD para ser tratado por outra função    
    nome_arquivo_html <- paste0(ano, "-", cod_entidade,
        ...
        if (file.exists(file.path("resposta_scraping_html", nm_municipio,
              ...
              nome_arquivo_html <- paste0(gsub("_.html", "", nome_arquivo_html), "-d_", sufixo, "_.html")
    ...
    print(paste("Scraping -", "Doc:",documento, "- Pág: ", pagina,
                 "- Ano: ", ano, "- Valor:", valor_documento, "-", nm_entidade))
...
```{r data_wrangling_html_despesas}
...
log_tratamento_arq_html_despesa <- log_data_hora()
...
tb_despesas_municipais <- tibble::tibble(
                                        ...
                                        hash_arq_html_despesa = hash_arq_html_despesa,
                                        log_tratamento_arq_html_despesa = log_tratamento_arq_html_despesa,
                                        link = link_despesa
                                        )
...
executar_tidy_data <- function() {
  ...
  tb_despesas_municipios <- DBI::dbReadTable(conect_bd, "tabela_despesas_municipais") %>%
                            ...
                            dplyr::mutate(valor_das_retencoes = valor_monetario(valor_das_retencoes)) %>%
                            dplyr::mutate(valor_do_pagamento = valor_monetario(valor_do_pagamento)) %>%
                            dplyr::mutate(valor_do_empenho = valor_monetario(valor_do_empenho)) %>%
                            dplyr::mutate(data_do_pagamento = lubridate::dmy(data_do_pagamento)) %>%
                            dplyr::mutate(data_empenho = lubridate::dmy(data_empenho)) %>%
                            dplyr::mutate_all(stringr::str_to_upper) %>%
                            dplyr::mutate(link = stringr::str_to_lower(link)) %>%
                            dplyr::mutate(nm_arq_html_despesa = stringr::str_to_lower(nm_arq_html_despesa)) %>%
                            dplyr::mutate(hash_arq_html_despesa = stringr::str_to_lower(hash_arq_html_despesa)) %>%
                            dplyr::mutate_all(stringr::str_trim) %>%
...
print("Todos os dados foram colocados no padrão Tidy Data no BD")


tb_despesas_municipios_BR <- tb_despesas_municipios %>%
                             dplyr::mutate(valor_das_retencoes = stringr::str_replace(valor_das_retencoes, "[.]", ",")) %>%
                             dplyr::mutate(valor_do_pagamento = stringr::str_replace(valor_do_pagamento, "[.]", ",")) %>%
                             dplyr::mutate(valor_do_empenho = stringr::str_replace(valor_do_empenho, "[.]", ","))


readr::write_delim(tb_despesas_municipios, file.path("bd_sqlite", "tabela_despesas_municipais_tidy_data.csv"),
                   delim = ";")

print("Todos os dados foram colocados no padrão Tidy Data internacional em CSV")

readr::write_delim(tb_despesas_municipios_BR, file.path("bd_sqlite", "tabela_despesas_municipais_tidy_data_BR.csv"),
                   delim = ";")

print("Todos os dados foram colocados no padrão Tidy Data Brasil (R$) em CSV")
...
# Função que cria a tabela das entidades alvos e faz o Web Scraping das páginas
# que têm os links que darão acesso as páginas HTML que contêm os dados sobre as despesas
executar_scraping_num_pags(anos_alvos, cod_municipios)
...
# BUG
# Identificar por que a tabela_entidades_alvo_paginas não está criando as paginações
# ao se acrescentar novos anos ex: "2017". O problema deve estar na função
# 'executar_scraping_num_pags'


# PRIORIDADES
#!!! Docker do RStudio no Digital Ocean
#!!! Generalizar a função scraping_html_purrr para realizar requisições e tratar erros de 404, Dos e timeout,
#    bem como gravar logs; bem como poder utilizá-la em todas etapas de scraping do código.



# MELHORIAS
#!!! Aperfeiçoar a execução em cascata do código (colocar função secundária dentro de função primária???)
#!!! Paralelizar requisições e o tratamento de dados com o pacote `furrr`
#!!! Atribuir cor aos prints
#!!! Analisar os comandos break, next ou return dentro da função purrr

# IMPLEMENTAÇÕES
#!!! Generalizar a função scraping_html_purrr para realizar requisições e tratar erros de 404, Dos e timeout,
#    bem como gravar logs; bem como poder utilizá-la em todas etapas de scraping do código.
#!!! Implementar uma função para identificar HTML com problemas/homissões no seu conteúdo devido a problemas
#    no servidor do TCM ao responder as requisições

#!!! Na função 'executar_tidy_data' especificar a saída do ID como numérico. E não torna maiúscula
#    as variáveis "nm_arq_html_despesa", hash_arq_html_despesa e link
#!!! Ajustar a data em 'pagamento' para o padrão inglês na função tidy_data
#!!! Terntar converter o encoding para UFT-8

#!!! Implementar uma Função de backup do BD
