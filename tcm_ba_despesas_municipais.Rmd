...
# na rotina antior
tb_municipios_alvos_novos <- git2r::merge(tb_municipios_alvos_novos, tb_dcalendario$ano) %>%
                             dplyr::mutate(ano = y) %>%
...
tb_municipios_alvos_atualizada <- tb_municipios_alvos_novos %>%
                                  ...
                                  dplyr::filter(!ano %in% ano_ant & cod_municipio %in% cod_municipios_alvos)
...
#!!! Identificar por que a tabela_entidades_alvo_paginas não está criando as paginações
#    ao se acrescentar novos anos ex: "2017". O problema deve estar na função
#    'executar_scraping_num_pags' ou na função que cria tabela_entidades_alvo_paginas
...
#!!! Na função 'executar_tidy_data' especificar a saída do ID como numérico.
#!!! Atribuir cor aos prints
#!!! Criar uma função específica para tratar da geração da 'tabela_entidades_alvos_paginas", que
#    atualmente está dentro da função 'executar_scraping_num_pags(anos_alvos, cod_municipios)'
