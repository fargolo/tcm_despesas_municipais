...
filtro_ant <- tb_municipios_alvos_anteriores$filtro
...
tb_municipios_alvos_atualizada <- tb_municipios_alvos_novos %>%
  ...
  # operador lógico para excluir os dados da coluna (Lei de De Morgan)
                                  dplyr::filter(!filtro %in% filtro_ant) %>%
...
executar_tidy_data <- function() {
  ...
  tb_despesas_municipios <- DBI::dbReadTable(conect_bd, "tabela_despesas_municipais") %>%
                            ...
                            dplyr::mutate_at(vars(valor_do_pagamento, valor_das_retencoes,
                                                  valor_do_empenho), funs(valor_monetario)) %>%
...
readr::write_delim(tb_despesas_municipios, file.path("dados_exportados", "tabela_despesas_municipais_tidy_data.csv"),
                   delim = ";")

print("Todos os dados foram colocados no padrão Tidy Data internacional em CSV")

readr::write_delim(tb_despesas_municipios_BR, file.path("dados_exportados", "tabela_despesas_municipais_tidy_data_BR.csv"),
                   delim = ";")
...
```{r executar, eval=FALSE, message=FALSE, include=FALSE}
