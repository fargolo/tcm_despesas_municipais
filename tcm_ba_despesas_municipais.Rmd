...
scraping_num_pags <- function(id, ano, cod_municipio, nm_municipio, cod_entidade, nm_entidade, pagina) {
  ...
  repeat {
    ...
    if (nm_arq_html_pag %in% tb_pag_links$nm_arq_html_pag & gatinho_to_break == 20) {
        ...
        print(paste("Fim das requisições de", nm_entidade, "na Página", pagina, "- T"))
...
parser_arq_html_pags <- function(id, ano, cod_municipio, nm_municipio,
  ...
  tb_requisicoes_anterior <- DBI::dbReadTable(conect_bd, "tabela_requisicoes") %>%
                             dplyr::filter(cod_entidade = cod_entidade)
...
DBI::dbWriteTable(conect_bd, "tabela_despesas_municipais", tb_gastos_municipais, append = TRUE)
...
executar_tidy_data <- function() {
  ...
  tb_gastos_municipios <- DBI::dbReadTable(conect_bd, "tabela_despesas_municipais") %>%
                          tibble::as.tibble() %>%
...
DBI::dbWriteTable(conect_bd, "tabela_despesas_municipais_tidy_data", tb_gastos_municipios, overwrite = TRUE)
  
readr::write_delim(tb_gastos_municipios, file.path("bd_sqlite", "tabela_despesas_municipais_tidy_data.csv"),
                   delim = ";")
...
# Função que faz o Web Scraping (via Web Service) do código e nome das Entidades.
...
# PRIORIDADES
#!!! Colocar rotina para evitar a sobrescrição dos arquivos, colocando o tempo de geração ao final.
#!!! Generalizar a função scraping_html_purrr para realizar requisições e tratar erros de 404, Dos e timeout,
#    bem como gravar logs
